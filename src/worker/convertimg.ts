...
(async () => {
    ...
    try {
        ...
        for (const item of queue_item) {
            ...
            if (params.from === 'jpeg' || params.from === 'jpg') {
                converted = await jimp(params.data_to_convert, params.filename);
                if (!converted) throw new Error('error converting data from jpg to png');

            } else {
                converted = await jimp(params.data_to_convert, params.filename)
                if (!converted) throw new Error('error converting data from png to jpg');
            }

            const upload = await s3({ data: converted, filename: params.filename });
        ...
        console.log('convert img error---', error);
